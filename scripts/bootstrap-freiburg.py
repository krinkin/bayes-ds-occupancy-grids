#!/usr/bin/env python3
"""Spatial block bootstrap 95% CIs for Freiburg 079 metric deltas.

Usage::

    source .venv/bin/activate
    python scripts/bootstrap-freiburg.py --config configs/freiburg-079/default.yaml
    python scripts/bootstrap-freiburg.py --config configs/freiburg-079/default.yaml --sensitivity

Loads the Freiburg 079 dataset, rebuilds probability maps for each arm,
...
Results are saved to results/freiburg-079/bootstrap-cis.json and
appended to results/freiburg-079/analysis-summary.md.
"""
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Spatial block bootstrap CIs for Freiburg 079 metric deltas"
    )
    ...
    data_path = cfg.get("data", {}).get("path", "data/freiburg-079/fr079-sm.log")
    ...
    output_dir = Path(cfg.get("output", {}).get("dir", "results/freiburg-079"))
    ...
    print("Freiburg 079 Spatial Block Bootstrap CI Analysis")
